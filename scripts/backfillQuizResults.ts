...
async function applyBatch(rows: Row[], batchNo: number) {
  ...
  console.log(`Batch #${batchNo}: fetched ${fetched}. Candidates to update: ${updates.length}.`);

  const lastId = rows[rows.length - 1].id;

  if (DRY_RUN) {
    updates.forEach(u => console.log(`[DRY_RUN] would update QuizResult ${u.id} -> userId=${u.userId}`));
